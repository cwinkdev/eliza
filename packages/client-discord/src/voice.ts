...
export class VoiceManager extends EventEmitter {
        ...
        try {
            ...
            const callback: HandlerCallback = async (content: Content) => {
                ...
                if (responseMemory.content.text?.trim()) {
                    ...
                    // Send text response to the channel instead of voice
    ...
    async playAudioStream(userId: UUID, audioStream: Readable) {
        console.log(
            "Starting playAudioStream with audio stream:",
            audioStream?.readable
        );
        ...
        if (connection.state.status !== "ready") {
            console.error(
                `Voice connection not ready. Current state: ${connection.state.status}`
            );
            try {
                console.log("Attempting to wait for ready state...");
                await entersState(
                    connection,
        ...
        console.log("Found voice connection, cleaning up previous player...");
        this.cleanupAudioPlayer(this.activeAudioPlayer);
        ...
        console.log("Successfully subscribed to audio player");
        ...
        try {
            ...
            audioPlayer.on("stateChange", (oldState, newState) => {
                ...
                if (newState.status === "idle") {
                    ...
                    console.log(
                        `Audio playback completed in: ${idleTime - audioStartTime}ms`
                    );
                }
            });
